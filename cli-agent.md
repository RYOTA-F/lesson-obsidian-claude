...
CLIベースのAIエージェント(Claude Code, Aider, GitHub Copilot CLI等)は、ターミナル環境で動作するAI支援ツール。GUIベースのAIチャット(ChatGPT, Claude Web等)とは異なる特性を持ち、エンジニアのワークフローに深く統合される。
...
3. CI設定(.github/workflows/review.yml)
   ```yaml
   - name: AI Code Review
     run: |
...
**理由**: エンジニアはプロジェクト全体のコンテキストをAIに理解させたい
...
- [CLIツール入門](../tool/cli-basics.md) - ターミナル操作の基礎
- [シェルスクリプティング](../dev/shell-scripting.md) - 自動化の実践
- [Git統合](../dev/git-integration.md) - バージョン管理との連携
